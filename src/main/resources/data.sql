...
CREATE TABLE tea_categories (
    ...
    name VARCHAR(100) NOT NULL UNIQUE COMMENT '种类名称',
...
CREATE TABLE admins (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    shop_name VARCHAR(50) UNIQUE NOT NULL COMMENT '商家用户名',
    password VARCHAR(255) NOT NULL COMMENT '密码',
    phone VARCHAR(20) COMMENT '手机号',
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    update_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
...
CREATE TABLE products (
    ...
    category ENUM('GREEN_TEA', 'OOLONG_TEA', 'BLACK_TEA', 'WHITE_TEA', 'PUERH_TEA') COMMENT '商品分类',
...
('zhangsan', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', '13800138001', 'zhangsan@example.com'),
('lisi', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', '13800138002', 'lisi@example.com');

-- 插入初始管理员数据
INSERT INTO admins (shop_name, password, phone) VALUES
('admin', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', '13800138000');

-- 插入初始商品数据
INSERT INTO products (product_name, description, price, category, stock, status) VALUES
-- 绿茶
...
-- 插入茶叶溯源示例数据
INSERT INTO tea_tracing (product_id, origin, harvest_time, process, tea_variety) VALUES
(1, '浙江省杭州市西湖区龙井村', '2025-03-15', '采摘后经萎凋、揉捻、发酵、干燥等工艺制成', '群体种'),
(2, '江苏省苏州市吴中区洞庭山', '2025-03-20', '采摘后经杀青、揉捻、搓团显毫、烘干等工艺制成', '碧螺春群体种'),
(3, '河南省信阳市浉河区', '2025-04-01', '采摘后经杀青、揉捻、解块、初烘、摊凉、复烘等工艺制成', '信阳群体种');

-- 插入茶叶品鉴示例数据
INSERT INTO tea_tasting (product_id, fragrance, taste, steep_resistance) VALUES
(1, '豆香', '鲜爽甘醇，回味悠长', 8),
(2, '花果香', '鲜醇甘甜，香气清雅', 7),
(3, '栗香', '鲜爽醇厚，回甘明显', 9);

-- 插入冲泡指南示例数据
INSERT INTO brewing_guide (product_id, water_temperature, tea_amount, brewing_time, brewing_times) VALUES
(1, 85, 3.00, 90, 3),
(2, 80, 2.50, 60, 4),
(3, 90, 3.50, 120, 5);

-- 插入茶叶种类示例数据
INSERT INTO tea_categories (name, description) VALUES
('绿茶', '未经发酵的茶叶，保留了鲜叶的天然物质'),
('乌龙茶', '半发酵茶，兼具绿茶和红茶的特点'),
('红茶', '全发酵茶，茶汤呈红色'),
('白茶', '轻微发酵茶，制作工艺最自然'),
('普洱茶', '后发酵茶，具有越陈越香的特点'),
('花草茶', '不含茶叶，由各种植物花朵、果实等制成'),
('茶具', '各类茶具用品');
...
# -- 插入初始茶器数据
# INSERT INTO tea_sets (name, description, price, material, category, status) VALUES
